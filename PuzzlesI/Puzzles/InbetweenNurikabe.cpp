...
namespace puzzles::InbetweenNurikabe{
...
int puz_state::find_matches(bool init)
{
    ...
    for (int r = 1; r < sidelen() - 1; ++r)
        for (int c = 1; c < sidelen() - 1; ++c) {
            Position p(r, c);
            if (char ch = cells(p); ch == PUZ_SPACE || ch == PUZ_EMPTY)
                space2hints[p];
        }
    ...
    for (auto& [p, v] : m_matches) {
        ...
        boost::remove_erase_if(v, [&](auto& kv2) {
            auto& [p2, perm_id] = kv2;
            auto& perm = m_game->m_pair2garden.at({min(p, p2), max(p, p2)}).m_perms[perm_id];
...
void solve_puz_InbetweenNurikabe()
{
    using namespace puzzles::InbetweenNurikabe;
    solve_puzzle<puz_game, puz_state, puz_solver_astar<puz_state>>(
        "Puzzles/InbetweenNurikabe.xml", "Puzzles/InbetweenNurikabe.txt", solution_format::GOAL_STATE_ONLY);
}
