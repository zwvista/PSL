...
struct puz_state3 : pair<set<Position>, set<Position>>
{
    puz_state3(const puz_game* game, const Position& p1, const Position& p2);

    void make_move(const Position& p1, const Position& p2) { first.insert(p1), second.insert(p2); }
    void gen_children(list<puz_state3>& children) const;
    
    set<Position> painting() {
        auto rng = first;
        rng.insert_range(second);
        return rng;
    }
                    ...
                    if (int area_id2 = m_pos2area.at(p2); area_id1 != area_id2) {
                        ...
                        for (auto& s : smoves) {
                            ...
                            if (boost::algorithm::any_of(m_moves, [&](const puz_move& move) {
                                return painting == move.m_painting;
                            })) continue;
                            ...
                            for (auto& p3 : painting)
                                for (auto& os2 : offset)
                                    if (auto p4 = p3 + os2; is_valid(p4) && !painting.contains(p4))
                                        empties.insert(p4);
                            m_moves.emplace_back(p1, p2, area_id1, area_id2, painting, empties);
                        }
                    }
        }
}
