...
/*
    iOS Game: 100 Logic Games/Puzzle Set 10/Tap-A-Row
...
puz_hint puz_state::compute_hint(const Position& p) const
{
    ...
    return puzzles::TapARow::compute_hint(filled);
}

bool puz_state::is_valid_move() const
{
    auto is_interconnected = [&]{
        int i = m_cells.find(PUZ_FILLED);
        if (i == -1)
            return true;
        ...
        }) == boost::count(m_cells, PUZ_FILLED);
    };
        ...
        for (int r = 1; r < sidelen() - 1; ++r) {
            ...
            for (int c = 1; c < sidelen() - 1; ++c) {
                ...
                if (ch == PUZ_HINT) {
                    ...
                    has_hint = true;
                    hinted_cnt += boost::accumulate(hint, 0);
                } else if (ch == PUZ_FILLED)
                    filled_cnt++;
            }
            if (has_hint && (!b && filled_cnt > hinted_cnt || b && filled_cnt != hinted_cnt))
                return false;
        }
        return true;
    };

    return is_interconnected() && is_valid_square({PUZ_FILLED}) && is_valid_row();
}
