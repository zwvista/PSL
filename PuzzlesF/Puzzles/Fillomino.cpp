		...
		for(auto& p : area.m_inner)
			for(int i = 0; i < 4; ++i){
				...
				if(walls.at(p_wall) != PUZ_WALL_ON && cells(p2) == PUZ_UNKNOWN && [&]{
					for(int j = 0; j < 4; ++j){
						auto p3 = p2 + offset[j];
						auto p_wall2 = p2 + offset2[j];
						auto& walls2 = j % 2 == 0 ? m_horz_walls : m_vert_walls;
						if(walls2.at(p_wall2) == PUZ_WALL_ON && is_valid(p3) &&
							cells(p3) == area.m_cell_count)
							return false;
					}
					return true;
				}())
