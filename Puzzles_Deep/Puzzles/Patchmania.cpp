...
void puz_state2::gen_children(list<puz_state2>& children) const
{
    ...
    if (!second && m_state->is_teleport(first))
        children.emplace_back(*this).make_move(m_state->get_teleport(first).second, true);
    else
        for (int i = 0; i < 4; ++i) {
            ...
            if (ch == m_info->m_food_name || ch == PUZ_MUSHROOM || ch == PUZ_SPACE &&
                ...
                children.emplace_back(*this).make_move(p, false);
        }
}
...
void puz_state::gen_children(list<puz_state>& children) const
{
    if (m_curr_bunny == 0)
        for (auto& [bunny, info] : m_bunny2info) {
            ...
            for (auto& kv : smoves)
                if (!children.emplace_back(*this).make_move(info.m_bunny, kv.first, kv.second))
                    children.pop_back();
        }
    else {
        auto& info = m_bunny2info.at(m_curr_bunny);
        ...
        if (!info.m_teleported && is_teleport(p1)) {
            if (!children.emplace_back(*this).make_move(p1, get_teleport(p1).second, true))
                children.pop_back();
        } else
            for (int i = 0; i < 4; ++i) {
                ...
                if (ch == PUZ_HOLE || ch == info.m_food_name || ch == PUZ_MUSHROOM ||
                    ...
                    if (!children.emplace_back(*this).make_move(p1, p2, false))
                        children.pop_back();
            }
    }
}
