...
/*
	...
	Description
	1. In kakuro you must fill the board with straight horizontal and
	   vertical lines(kakuro) that stem from each number.
	2. The number itself tells you how many tiles its kakuro fill up.
	   The tile with the number doesn't count.
	3. There can't be blank tiles and kakuro can't overlap, nor run over
	   the numbers. Moreover kakuro must be in a single straight line
...
struct puz_state : string
{
	...
	unsigned int get_heuristic() const { return m_pos2kakuro.size(); }
	...
	map<Position, vector<vector<int>>> m_pos2kakuro;
};
...
{
	const auto& kv = *boost::min_element(m_pos2kakuro,
