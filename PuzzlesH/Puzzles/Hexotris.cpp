...
{
    puz_state2(const puz_game& game, const puz_region& region, const Position& p, const Position& p2)
        : m_game(&game), m_region(&region), m_p2(&p2) {make_move(p);}

    bool is_goal_state() const { return m_distance == m_region->m_num; }
    bool make_move(const Position& p) {
        insert(p); ++m_distance;
        // cannot go too far away
        return boost::algorithm::any_of(*this, [&](const Position& p2) {
            return manhattan_distance(p2, *m_p2) <= m_region->m_num - m_distance;
        });
    }
...
void puz_state2::gen_children(list<puz_state2>& children) const {
    for (auto& p : *this)
        for (auto& os : offset) {
            ...
            // if it is already in the region
...
, m_sidelen(strs.size() + 2)
{
    ...
    for (auto& [p, n] : m_pos2num)
        for (auto& [p2, n2] : m_pos2num) {
            ...
            region.m_num = n3;
            puz_state2 sstart(*this, region, p, p2);
...
struct puz_state
{
    ...
    // value.elem.second: index into the permutations (forms) of the region
...
{
    // key: a space tile
    // value.elem: positions of the two numbers from which a region
