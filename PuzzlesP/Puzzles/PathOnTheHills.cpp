...
bool puz_state::check_loop() const
{
    ...
    // 2. The object is to have a walk around the Countryside, passing through
    //    each Field just once.
    for (auto& [_1, area] : m_game->m_areas) {
        int num = 2;
        int max_possible = 0;
...
void solve_puz_PathOnTheHills()
{
    using namespace puzzles::PathOnTheHills;
    solve_puzzle<puz_game, puz_state, puz_solver_astar<puz_state>>(
        "Puzzles/PathOnTheHills.xml", "Puzzles/PathOnTheHills.txt", solution_format::GOAL_STATE_ONLY);
}
