...
namespace puzzles{ namespace zafiro{
...
unsigned int puz_state::get_heuristic() const
{
	...
	//if(ir == jr){
	//	int d = ic < jc ? 1 : -1;
	//	if(cell(Position(jr, jc + d)) != PUZ_SPACE) return 3;
	//	for(int c = ic + d; c != jc; c += d)
	//		switch(cell(Position(ir, c))){
	//		case PUZ_FIXED:
	//			return 3;
	//		case PUZ_GLASS:
	//			++num_glass;
	...
	//else{
	//	int d = ir < jr ? 1 : -1;
	//	if(cell(Position(jr + d, jc)) != PUZ_SPACE) return 3;
	//	for(int r = ir + d; r != jr; r += d)
	//		switch(cell(Position(r, ic))){
	//		case PUZ_FIXED:
	//			return 3;
	//		case PUZ_GLASS:
	//			++num_glass;
